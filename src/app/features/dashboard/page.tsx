        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-gray-600 text-sm leading-relaxed mb-4">
              Traditionally, tracking the status of twenty or thirty applicants means chasing emails, updating spreadsheets, and asking colleagues whether they’ve seen a particular result. Evalent replaces all of that with a single, automatically updated dashboard.
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: '●', label: 'Live updates', desc: 'Status changes instantly on completion' },
                { icon: '■', label: 'Score breakdown', desc: 'English, Maths, Reasoning per student' },
                { icon: '▲', label: 'Recommendation', desc: 'Ready to admit, support, waitlist' },
                { icon: '◆', label: 'Full report', desc: 'One click from dashboard to PDF' },
              ].map((f) => (
              ...
              <div className="bg-navy rounded-2xl overflow-hidden aspect-video flex items-center justify-center group cursor-pointer relative">
                ...
                <div className="relative text-center z-10">
                  ...
                  <div className="text-white font-bold text-sm mb-1">Watch: Assessment Dashboard</div>
      ...
      <section className="py-14 px-6 bg-white border-t border-gray-100">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { status: 'Registered',   cls: 'bg-gray-100 text-gray-600',   dot: 'bg-gray-400',    desc: 'Student added to the system. Assessment link not yet sent (unusual — link normally sends on registration).' },
              { status: 'Link sent',    cls: 'bg-yellow-100 text-yellow-700',dot: 'bg-yellow-500',  desc: 'The secure assessment link has been emailed. Evalent is monitoring for completion and will send reminders.' },
              { status: 'In progress',  cls: 'bg-blue-100 text-blue-700',   dot: 'bg-blue-500',    desc: 'The student has opened the link and begun their assessment. The timer is running.' },
              ...
              { status: 'Expired',      cls: 'bg-red-100 text-red-600',     dot: 'bg-red-400',     desc: 'The 72-hour link has expired without completion. You are notified and can choose to extend or proceed.' },
              { status: 'Decision made',cls: 'bg-purple-100 text-purple-700',dot:'bg-purple-500',  desc: 'The assessor has recorded their decision. The applicant record is final and audit-logged.' },
            ].map((s) => (
      ...
      <section className="py-14 px-6 bg-white border-t border-gray-100">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              ['Can I filter by grade or curriculum?', 'Yes — the dashboard can be filtered by grade, curriculum, and status. This is particularly useful for schools running assessments across multiple entry points simultaneously.'],
              ['Can multiple admin users see the dashboard?', 'All school admin users on your Evalent account have dashboard access. Permissions can be set by role.'],
              ['Can I export the data?', 'CSV export is available from the dashboard. This is useful for integration with your existing admissions CRM or for sharing with board members.'],
              ['How quickly do scores appear after completion?', 'Scores are calculated and the report is generated automatically. The dashboard updates within seconds of the student submitting their assessment.'],
            ].map(([q, a]) => (
